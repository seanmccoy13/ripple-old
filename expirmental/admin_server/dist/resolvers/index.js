...
exports.default = {
  ...
  Query: {
    scheduleTest: async () => await (0, _scheduler.testSchedule)(),
  ...
  Mutation: {
    ...
    createCompany: async (_, { companyName, schedule, groups }, { models: { Company, Employee } }) => {
      ...
      try {
        ...
        if (schedule) {
          (0, _scheduler.addSchedule)({ companyName, Employee, schedule });
          (0, _scheduler.startSchedule)({ companyName });
        } else {
          (0, _scheduler.addSchedule)({ companyName, Employee, schedule: "30 8 * * Fri" });
          (0, _scheduler.startSchedule)({ companyName });
        }
      } catch (error) {
    ...
    editCompany: async (_, { companyName, groups, schedule }, { models: { Company, Employee } }) => {
      const exists = await Company.findOne({ companyName }).exec();
      if (!exists) {
      ...
      try {
        ...
        if (schedule) {
          (0, _scheduler.stopSchedule)({ companyName });
          (0, _scheduler.destorySchedule)({ companyName });
          (0, _scheduler.addSchedule)({ companyName, Employee, schedule });
          (0, _scheduler.startSchedule)({ companyName });
        }
        return null;
    ...
    forgotPassword: async (_, { email }, { models: { User }, url }) => {
      ...
      const sentEmail = await (0, _sendEmail.sendEmail)({ email, url, resetId });
      if (!sentEmail) {
        return (0, _createError2.default)("email", "forgot password send email failed");
      }
      return null;
    }
  }
